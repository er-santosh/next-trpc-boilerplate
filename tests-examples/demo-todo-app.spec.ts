...
async function checkNumberOfTodosInLocalStorage(page: Page, expected: number) {
  return await page.waitForFunction(
    e => (JSON.parse(localStorage['react-todos']) as any[]).length === e,
    expected
  );
}
...
async function checkNumberOfCompletedTodosInLocalStorage(page: Page, expected: number) {
  return await page.waitForFunction(
    e =>
      (JSON.parse(localStorage['react-todos']) as any[]).filter((todo: any) => todo.completed)
        .length === e,
    expected
  );
}
...
async function checkTodosInLocalStorage(page: Page, title: string) {
  return await page.waitForFunction(
    t =>
      (JSON.parse(localStorage['react-todos']) as any[]).map((todo: any) => todo.title).includes(t),
    title
  );
}
